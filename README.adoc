= 🚀 Galaxium Travels - Interplanetary Booking System


A complete full-stack application for booking interplanetary space travel, featuring a modern React frontend and 
Quarkus backend with REST and ChatBot support.
The Hold Service (Reservations) is implemented in Quarkus.

== 🌟 Features

Modern Space-Themed UI:: - Beautiful, responsive interface with animated starfield
Full Booking System:: - Browse flights, make bookings, manage reservations
Three Seat Classes:: - Economy, Business, and Galaxium Class with independent availability tracking
Dynamic Pricing:: - Class-based multipliers (1x, 2.5x, 5x) applied to base flight prices
Type-Safe:: - Full TypeScript frontend and Java type hints with strict validation
Real-Time Updates:: - Live seat availability per class and booking status
User Management:: - Simple name/email authentication
AI:: - Chat Bot integration / MCP / A2A / Human-In-The-Loop / Docling

== 🚀 Manual Quick Start

* NodeJS 18+
* Java 21+
* Podman/Docker

=== Frontend

[source, bash]
----
cd booking_system_frontend
----

Create a `.env` file with the following content:

[source]
----
VITE_API_URL=http://localhost:8001
----

Then:

[source, bash]
----
npm install
npm run dev
----

=== Backend

[source, bash]
----
cd booking-system-backend-quarkus
./mvnw quarkus:dev
----

IMPORTANT: You need to have Podman/Docker (Desktop) installed to run this demo. Quarkus application will start the PostgreSQL container automatically.

=== Hold Service

[source, bash]
----
cd inventory-hold-service-quarkus 
./mvnw quarkus:dev
----

You *only* need this service if you plan to book flights.

=== Time Dilation MCP Server

[source, bash]
----
cd time-dilation-calculator
./mvnw quarkus:dev
----

You *only* need to start this server if you plan to use the ChatBot to calculate the time dilation in the ChatBot.

=== Incidence Manager

To start the incidence manager, first you need to start the Incidence Response Agent:

[source, bash]
----
cd incidence-response-agent
./mvnw quarkus:dev
----

And then you can start the Incidence Manager:

[source, bash]
----
cd incidence-manager
./mvnw quarkus:dev
----

You *only* need these services if you plan to use the Customer Support.

=== Notification Manager

[source, bash]
----
cd notification-manager
./mvnw quarkus:dev
----

You *only* need this service if you plan to test notifications.

Then you can go to the browser http://localhost:5173/ and start using the app.
