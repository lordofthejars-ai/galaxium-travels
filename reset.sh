...
# 
# This script cleans up after completing the demo instructions
...
if [[ ! -f "README.adoc" ]] || [[ ! -d "booking-system-backend-quarkus" ]] || [[ ! -d "inventory-hold-service-quarkus" ]]; then
...
DB_FILES=("booking-system-backend-quarkus/booking.db" "inventory-hold-service-quarkus/holds-db.mv.db" "booking.db" "*.db-journal")
...
# Java backend service cleanup
if [[ -d "booking-system-backend-quarkus/target" ]]; then
    print_info "Cleaning Java service artifacts..."
    rm -rf booking-system-backend-quarkus/target
    print_success "Cleaned Java service artifacts"
    CLEANED_ITEMS+=("Cleaned Java backend service artifacts")
fi

# Java inventory hold service cleanup
if [[ -d "inventory-hold-service-quarkus/target" ]]; then
    print_info "Cleaning Java service artifacts..."
    rm -rf inventory-hold-service-quarkus/target
    print_success "Cleaned Java inventory hold service artifacts"
    ((BACKEND_CLEANED++))
